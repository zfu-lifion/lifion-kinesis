...
describe('lib/polling-consumer', () => {
  ...
  const getRecords = jest.fn(() =>
    Promise.resolve({
      ...
      Records: [{ Data: 'foo', SequenceNumber: 1, ApproximateArrivalTimestamp: timestamp }]
    })
  );
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout.mockImplementationOnce(() => {
        try {
          ...
          expect(storeShardCheckpoint).toHaveBeenCalledWith('shardId-0000', 1, '', {}, timestamp);
  ...
  test('the consumer calls the deaggregate function if included in options', () => {
    return new Promise((resolve, reject) => {
      ...
      setTimeout.mockImplementationOnce(() => {
        try {
          expect(deaggregate).toHaveBeenCalledWith([{ Data: 'foo', SequenceNumber: 1, ApproximateArrivalTimestamp: timestamp }]);
          expect(getShardIterator).toHaveBeenCalledWith({
            ShardId: 'shardId-0000',
            ShardIteratorType: 'LATEST',
            StreamName: 'stream'
      ...
      pushToStream.mockImplementationOnce(async (err, { setCheckpoint }) => {
        if (!err) {
          try {
            ...
            expect(storeShardCheckpoint).not.toHaveBeenCalled();
            await setCheckpoint('123', timestamp);
            expect(storeShardCheckpoint).toHaveBeenCalledWith('shardId-0000', '123', '', {}, timestamp);
            expect(debug.mock.calls).toEqual([
              ['Starting to read shard "shardId-0000" from the latest record.'],
              ['Got 1 record(s) from "shardId-0000" (10ms behind)']
            ]);
