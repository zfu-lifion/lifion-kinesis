...
describe('lib/polling-consumer', () => {
  ...
  test('shard iterator defaults to empty string if getShardIterator returns undefined', async () => {
    return new Promise((resolve, reject) => {
      getShardIterator.mockResolvedValueOnce({});
      ...
      setTimeout.mockImplementationOnce(() => {
        try {
          ...
          expect(getRecords).toHaveBeenCalledWith({ Limit: 1000, ShardIterator: '' });
          resolve();
        } catch (err) {
          reject(err);
        } finally {
