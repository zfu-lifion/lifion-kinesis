...
async function pollForRecords(instance) {
  ...
  try {
    ...
    if (seqNumToCheckpoint) {
      await setCheckpoint(seqNumToCheckpoint, timestamp);
      privateProps.seqNumToCheckpoint = null;
      privateProps.timestamp = null;
    }
    ...
    if (useAutoCheckpoints) {
      const { sequenceNumber, approximateArrivalTimestamp } = records[recordsCount - 1];
      console.log("-------------- sequenceNumber:", sequenceNumber);
      console.log("-------------- approximateArrivalTimestamp:",approximateArrivalTimestamp);
      if (!usePausedPolling) {
        await setCheckpoint(sequenceNumber, approximateArrivalTimestamp);
      } else {
        privateProps.seqNumToCheckpoint = sequenceNumber;
        privateProps.timeStamp = approximateArrivalTimestamp;
      }
    }
...
class PollingConsumer {
  ...
  async start() {
    ...
    privateProps.setCheckpoint = async (sequenceNumber, timestamp) => {
      console.log("-------- setCheckPoint func");
      await stateStore.storeShardCheckpoint(shardId, sequenceNumber, shardsPath, shardsPathNames, timestamp);
      privateProps.checkpoint = sequenceNumber;
      privateProps.timestamp = timestamp;
    };
