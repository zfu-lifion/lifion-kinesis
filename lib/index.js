...
async function setUpEnhancedConsumers(instance) {
  ...
  const newEnhancedConsumerBatches = newEnhancedConsumerNames.reduce(
    /**
     *
     * @param {Array<Array<string>>} batches - Batches
  ...
  for (const batch of newEnhancedConsumerBatches) {
    await Promise.all(
      batch.map((/** @type {string} */ consumerName) =>
 ...
 *
 * @param {Object.<string, string>} param - Output for the put record operation.
 ...
 *
 * @param {Object.<string, any>} param - Output for the put records operation.
...
function parsePutRecordsResult({ EncryptionType, Records }) {
  return {
    ...
    records: Records.map(
       ...
       * @param {string} param.SequenceNumber - The shard sequence number.
       * @param {string} param.ShardId - The shard id.
       * @returns {Object}
       */
...
class Kinesis extends PassThrough {
   ...
   * @param {Function} [options.logger.debug] - Logger debug message.
   * @param {Function} [options.logger.error] - Logger error message.
   * @param {Function} [options.logger.warn] - Logger warn message.
  ...
  async putRecord(params = {}) {
    const privateProps = internal(this);
    /** @type {{ client: KinesisClient, createStreamIfNeeded: boolean }} */
    ...
    try {
      const putRecordOutput = (await client.putRecord(awsParams)) || {};
      return parsePutRecordResult(putRecordOutput);
    } catch (/** @type {any} */ err) {
      const { code } = err;
   ...
   *
   * @param {Object} params - The parameters.
   * @param {Object[]} [params.records] - The records associated with the request.
