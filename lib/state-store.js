...
class StateStore {
   ...
   *
   * @param {Date} approxArrival - The approximate arrival timestamp
   ...
   * @returns {Promise}
   */
  async storeShardCheckpoint(approxArrival, shardId, checkpoint, shardsPath, shardsPathNames) {
    const privateProps = internal(this);
    const { logger } = privateProps;
    if (typeof checkpoint !== 'string') throw new TypeError('The sequence number is required.');
    let approxArrivalString = null;
    if (approxArrival instanceof Date) {
      approxArrivalString = approxArrival.toISOString();
    } else if (approxArrival) {
      logger.warn(
