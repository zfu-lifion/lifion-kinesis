...
async function getStreamState(instance) {
  ...
  /** @type {{ client: DynamoDbClient, consumerGroup: string, streamName: string }} */
...
class StateStore {
      ...
      try {
        await client.update({
          ...
          ExpressionAttributeNames: {
            '#a': 'consumers',
            '#b': 'version',
            ...Object.fromEntries(oldConsumers.map((id, index) => [`#${index}`, id]))
          },
          ExpressionAttributeValues: {
            ':x': generate(),
            ':y': version
   ...
   * @param {Object.<string, any>} shardData - The data describing the shard as returned by the AWS Kinesis API.
   * @param {StreamState} [streamState] - The current stream state, if known.
   * @fulfil {undefined}
   * @returns {Promise<void>}
   */
   ...
   * @returns {Promise<Object<string, EnhancedConsumer>>}
   */
  ...
  async getOwnedShards() {
    ...
    return Object.fromEntries(
      Object.keys(shards)
        .filter((shardId) => {
          const { depleted, leaseExpiration, leaseOwner } = shards[shardId];
          return (
            leaseOwner === consumerId &&
            new Date(leaseExpiration).getTime() - Date.now() > 0 &&
            !depleted
          );
        })
        .map((shardId) => {
          const { checkpoint, leaseExpiration, version } = shards[shardId];
          return [shardId, { checkpoint, leaseExpiration, version }];
        })
    );
  }
   ...
   *
   * @param {StreamState} [streamState] - The current state for the entire stream, if not provided,
   ...
   * @fulfil {ShardData} - An object containing `shards` (the current shards state), `shardsPath`
   ...
   * @param {StreamState} streamState - The known stream state.
   ...
   *
   * @param {Object.<string, Shard>} shardsData - The current shards state.
  ...
  async markShardAsDepleted(shardsData, parentShardId) {
    ...
    await client.update({
      ExpressionAttributeNames: {
        ...
        ...Object.fromEntries(
          childrenShards.map((childShard, index) => [`#${index}`, childShard.shardId])
        )
      },
      ExpressionAttributeValues: {
        ':x': true,
        ':y': generate(),
        ...Object.fromEntries(
          childrenShards.flatMap((childShard, index) => [
...
/**
 * @typedef ShardData
 * @property {Object.<string, Shard>} shards - Dictionary of shard objects.
 * @property {string} shardsPath - Shard path template.
 * @property {Object.<string, string>} shardsPathNames - Mapping of template keys to shard paths.
 */

/**
 * @typedef StreamState
 * @property {string} consumerGroup - Consumer group.
 * @property {Object.<string, Consumer>} consumers - Dictionary of consumer objects.
 * @property {Object.<string, EnhancedConsumer>} enhancedConsumers - Dictionary of enhanced consumer objects.
 * @property {Object.<string, Shard>} shards - Dictionary of shard objects.
 * @property {?string} streamCreatedOn - Timestamp string of stream creation date.
...
/**
 * @typedef Shard
 * @property {?string} checkpoint - Shard checkpoint string.
 * @property {boolean} depleted - Whether the shard has been depleted.
 * @property {string} leaseExpiration - Timestamp of when the lease expires.
 * @property {string} leaseOwner - Lease owner.
 * @property {string} parent - Parent shard.
 * @property {string} version - Version.
 * @property {?string} [startingSequenceNumber] - Starting sequence number.
 */

/**
 * @typedef Consumer
 * @property {string} appName - Consumer app name.
 * @property {string} heartbeat - Heartbeat.
 * @property {string} host - Host string.
 * @property {boolean} isActive - Whether the consumer is active.
 * @property {boolean} isStandalone - Whether this is a standalone consumer.
 * @property {number} pid - Process id.
 * @property {string} startedOn - Timestamp string of when the consumer was started.
 * @property {Object.<string, Shard>} shards - Dictionary of shard objects.
 */

/**
 * @typedef EnhancedConsumer
 * @property {string} arn - The enhanceed consumer's Amazon resource name.
 * @property {boolean} isStandalone - Whether this  is a standalone enhanced consumer.
 * @property {string} isUsedBy - The id of who this enhanced consumer is used by.
 * @property {Object.<string, Shard>} shards - Dictionary of shard objects.
 * @property {string} version - Version.
 */
