...
class StateStore {
   ...
   * @param {Date} timestamp - The approximate arrival timestamp
   * @fulfil {undefined}
   * @returns {Promise}
   */
  async storeShardCheckpoint(shardId, checkpoint, shardsPath, shardsPathNames, timestamp) {
    const privateProps = internal(this);
    ...
    if (timestamp instanceof Date) {
      ...
      logger.warn("Will ignore timestamp provided, it was not a date: " + JSON.stringify(timestamp)); 
    } else {
      logger.warn("Will ignore timestamp provided, it was null"); 
    }
