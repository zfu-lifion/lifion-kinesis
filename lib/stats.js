 ...
 * @returns {Object.<string, any>} The store for the stats.
 * @private
 */
 ...
 * @param {any} params.exception - The exception to store.
 * @param {Object.<string, any>} params.store - The object where to store the exception.
 * @private
 */
 ...
 *
 * @param {Cache<string, any>} cache - The cache instance where the exceptions are stored.
 * @returns {Object}
 * @private
 */
...
/**
 * @param {Array<any>} param - Parameter array.
 * @returns {Array<any>}
 */
...
function getStats(streamName) {
  ...
  const formattedStats = {
    version,
    ...Object.fromEntries(Object.entries(moduleStats).map(formatValues))
  };

  if (streamName) {
    const { recentExceptions, ...baseStats } = formattedStats;
    const streamData = getStore(streamName);
    baseStats.kinesis = Object.fromEntries(Object.entries(streamData).map(formatValues));
    return baseStats;
  }

  const formattedStreams = Object.fromEntries(
    Object.entries(streams).map(([key, value]) => {
      const finalValue = Object.fromEntries(Object.entries(value).map(formatValues));
 ...
 * @param {any} exception - The error to store.
