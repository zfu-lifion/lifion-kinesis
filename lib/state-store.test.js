...
describe('lib/state-store', () => {
  ...
  test('storeShardCheckpoint throws if called with no timestamp', async () => {
    const store = new StateStore(options);
    await store.start();
    await expect(
      store.storeShardCheckpoint('shard-0001', '1', '#a', { '#a': 'a' }, null)
    ).rejects.toThrow('The timestamp is required.');
  });
  ...
  test('storeShardCheckpoint updates the checkpoint of a shard', async () => {
    ...
    await store.start();
    await expect(
      store.storeShardCheckpoint('shard-0001', '1', '#a', { '#a': 'a' }, timestamp)
    ).resolves.toBeUndefined();
    const { update } = new DynamoDbClient();
    expect(update).toHaveBeenNthCalledWith(1, {
      ExpressionAttributeNames: {
