...
describe('lib/index', () => {
  ...
  describe('listShards', () => {
    test('list shards defaults to empty array on undefined output', async () => {
      new KinesisClient().listShards.mockImplementationOnce(() => ({}));
      const kinesis = new Kinesis(options);
      try {
        await kinesis.startConsumer();
  ...
  describe('putRecord', () => {
    test('defaults putRecord output to empty object if returned as undefined', async () => {
      const kinesis = new Kinesis(options);
      new KinesisClient().putRecord.mockImplementationOnce(() => {});
      try {
        await kinesis.startConsumer();
    ...
    test('defaults putRecord output to empty object if returned as undefined on catch', async () => {
      const kinesisClient = new KinesisClient();
      const kinesis = new Kinesis(options);
      kinesisClient.putRecord.mockRejectedValueOnce(
        Object.assign(new Error('foo'), { code: 'ResourceNotFoundException' })
      );
      kinesisClient.putRecord.mockImplementationOnce(() => {});
      try {
        await kinesis.startConsumer();
        const promise = kinesis.putRecord({ data: 'foo' });
        jest.runOnlyPendingTimers();
